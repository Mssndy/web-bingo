...
import { generateTossGrid, checkTossLines, calcTossScore, TOSS_BALLS } from '@/lib/toss';
import { playToss, playLand, playBingo } from '@/lib/sounds';
...
function Cell({
  ...
  const borderColor = inLine ? '#B45309' : marked ? '#451A03' : '#92400E';
  const shadow = marked
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: justLanded ? 'cell-catch 0.38s cubic-bezier(0.34,1.56,0.64,1) both' : undefined,
...
export default function TossGameScreen({ playerName, onHome }: Props) {
  ...
  const [ballIdSeq, setBallIdSeq] = useState(0);
  ...
  const handleThrow = useCallback((row: number, col: number) => {
    ...
    setTimeout(() => {
      const newMarked     = markedRef.current.map(r => [...r]);
  ...
  }, [flyingBall, phase, grid, ballIdSeq]);
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-5 animate-[fade-in_0.3s_ease_both]">
      ...
      {/* ── Flying ball overlay (position:fixed, above everything) ── */}
      {flyingBall && (
        <div
          key={flyingBall.id}
          style={{
            ...
            fontSize: 28,
            lineHeight: 1,
            // CSS custom props for the keyframe arc calculation
